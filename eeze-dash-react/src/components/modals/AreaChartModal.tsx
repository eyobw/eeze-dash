...
export default function AreaChartModal({ open, onClose }: Props) {
    ...
    return state.data.filter((row) => {
      for (const [key, val] of Object.entries(filter)) {
        if (val !== undefined && val !== '' && row[key] !== val) {
          if (typeof row[key] === 'number' && String(row[key]) !== String(val)) return false;
          if (typeof row[key] !== 'number' && row[key] !== val) return false;
        }
      }
      return true;
    });
